...
The code is small, but the boundaries are in useful places: MQTT lives in `backend/mqtt/client.py`, current telemetry lives in `FleetState`, and the UI is in `frontend/src/App.jsx`. That means a straightforward feature does not need a rewrite. For example, a robot detail/debug view could use the existing `GET /robots/{robot_id}` route in `backend/api/rest.py:get_robot()`. A click on a card in `App.jsx` could remember the selected ID, fetch that route, and show the recorded `t` value that the main dashboard intentionally leaves out. The MQTT topic and shared-state design could stay exactly as they are.
...
Today, if a robot goes quiet, the system keeps its last accepted event in `FleetState`. `frontend/src/App.jsx` continues to show those last values, and its “reporting” count is not a health check. That is an honest limitation of the current build. The source recording is telemetry, and inventing an `offline` event just because no message arrived would be misleading.
